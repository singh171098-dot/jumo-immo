...
export default function PropertyForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!selectedDpe) { setError("Veuillez sélectionner le DPE."); return; }
    setLoading(true);
    setError(null);
        ...
        <div className="p-8 space-y-6">
          <div className={`rounded-2xl border p-5 flex items-center justify-between ${SCORE_BG(score)}`}>
            <div>
              ...
              <p className={`text-4xl font-black tracking-tighter ${SCORE_COLOR(score)}`}>{score}<span className="text-base font-bold text-slate-400">/100</span></p>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => { setResult(null); setSelectedDpe(""); setSelectedType("Appartement"); setError(null); }}
    ...
    >
      ...
      <div className="px-8 py-6 border-b border-slate-100 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-xs mt-0.5 font-medium">Estimée en moins de 2 minutes · DVF automatique</p>
        </div>
      </div>
          ...
          <div className="flex gap-2 flex-wrap">
            {DPE_CONFIG.map(d => (
              <button
                key={d.label}
                type="button"
                onClick={() => setSelectedDpe(d.label)}
                disabled={loading}
      ...
      <div className="px-8 pb-7">
        ...
        <p className="text-center text-xs text-slate-400 mt-3 font-medium">
          0% de commission · Données DVF officielles · Visible immédiatement
        </p>
      </div>
    </form>
  );
