...
// Extends SavedProperty with a match score computed server-side
...
function RecommendationCard({ p, index }: { p: RecommendedProperty; index: number }) {
  ...
  return (
    ...
    >
      <Link
        href={`/annonces/${p.id}`}
        className="group block rounded-2xl bg-white/[0.04] border border-white/10 overflow-hidden hover:border-emerald-500/30 hover:bg-white/[0.06] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-emerald-900/20"
      >
        {/* Image */}
        ...
        <div className="p-4">
          ...
          <p className="flex items-center gap-1 text-gray-500 text-xs mb-2.5">
  ...
  return (
    <>
      ...
      >
        ...
        {/* ══════════════════════════════════
            BUYER PROJECT BANNER
            State A — no criteria
            State B — criteria defined
        ══════════════════════════════════ */}
        <motion.div variants={fadeUp}>
          {criteria ? (
            /* State B — criteria defined */
            <div className="rounded-2xl bg-gradient-to-r from-blue-500/[0.08] to-emerald-500/[0.06] border border-blue-500/20 p-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  ...
                  className="shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-400 text-sm font-bold hover:bg-blue-500/20 transition"
                >
                  <Pencil size={13} />
                  Modifier
                </button>
            ...
            <div className="rounded-2xl bg-gradient-to-br from-blue-500/[0.07] via-transparent to-emerald-500/[0.05] border border-white/10 p-6 text-center">
              ...
              <h2 className="text-xl font-black text-white mb-2">
                🎯 Définissez votre projet d'achat
              </h2>
              <p className="text-gray-500 text-sm mb-5 max-w-md mx-auto leading-relaxed">
                Laissez notre moteur d'analyse identifier les meilleures opportunités et les biens les plus attractifs selon vos critères.
              </p>
              <button
                onClick={() => setModalOpen(true)}
                className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-[#1E3A8A] to-blue-500 hover:from-blue-900 hover:to-blue-400 text-white font-bold rounded-xl text-sm transition-all shadow-lg shadow-blue-900/30 hover:-translate-y-0.5"
              >
                <Sparkles size={15} />
        ...
        >
                ...
                <p className="text-[10px] text-gray-600">
                  {criteria
                    ? recommendedProperties.length > 0
                      ? `${recommendedProperties.length} bien${recommendedProperties.length > 1 ? "s" : ""} correspondant à votre projet · Classés par FairScore`
          ...
          {!criteria && (
            <div className="text-center py-8 border border-dashed border-white/[0.08] rounded-xl">
              ...
              <p className="text-gray-700 text-xs mb-4">
                Définissez votre projet pour débloquer les opportunités personnalisées.
              </p>
              <button
                onClick={() => setModalOpen(true)}
                className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500/10 border border-blue-500/20 text-blue-400 rounded-xl text-sm font-bold hover:bg-blue-500/20 transition"
              >
          ...
          {/* State B — criteria exist but no matches */}
          {criteria && recommendedProperties.length === 0 && (
            <div className="text-center py-8 border border-dashed border-white/[0.08] rounded-xl">
              ...
              <p className="text-gray-500 text-xs max-w-xs mx-auto leading-relaxed mb-5">
                ...
                Nous analysons continuellement les nouvelles annonces et vous alerterons dès qu'une opportunité pertinente apparaîtra.
              </p>
              <button
                onClick={() => setModalOpen(true)}
                className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500/10 border border-blue-500/20 text-blue-400 rounded-xl text-sm font-bold hover:bg-blue-500/20 transition"
          ...
          {/* State A — recommendations available */}
          {criteria && recommendedProperties.length > 0 && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {recommendedProperties.map((p, i) => (
                  <RecommendationCard key={p.id} p={p} index={i} />
                ))}
              </div>
              ...
              return (
                <div key={step.key} className="flex items-start flex-1">
                  <div className="flex flex-col items-center flex-shrink-0" style={{ minWidth: 0, width: "100%" }}>
                    ...
                    <p className={`text-xs font-black text-center leading-snug px-2 ${isDone ? "text-emerald-400" : isActive ? "text-white" : "text-gray-600"}`}>
                      {step.label}
                    </p>
                    <p className={`text-[10px] text-center mt-0.5 px-2 leading-snug ${isDone ? "text-emerald-600" : isActive ? "text-gray-400" : "text-gray-700"}`}>
                      {step.desc}
                    </p>
            ...
            {[
              {
                ...
                value: total > 0 ? `${avgPricePerSqm.toLocaleString("fr-FR")} €` : "—",
                sub:   total > 0 ? "Moyenne de vos favoris" : "Sauvegardez des biens",
                color: "text-blue-400",
              },
              { label: "Évolution du marché local", value: "+2,3 %",  sub: "Sur 12 mois glissants",  color: "text-emerald-400" },
              {
                ...
                value: String(total),
                sub:   total === 1 ? "bien sauvegardé" : "biens sauvegardés",
        ...
        >
          ...
          {total === 0 ? (
            <p className="text-gray-600 text-sm text-center py-4">
              Sauvegardez des biens depuis les annonces pour voir vos analytics.
            </p>
          ) : (
            <>
        ...
        {/* ══════════════════════════════════
            MES FAVORIS — saved properties grid
        ══════════════════════════════════ */}
        <motion.div
          variants={fadeUp}
          className="rounded-2xl bg-white/[0.04] backdrop-blur-sm border border-white/10 p-6"
          ...
          <div className="rounded-2xl bg-gradient-to-r from-amber-500/[0.08] to-orange-500/[0.05] border border-amber-500/20 p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-500 text-xs mt-0.5">
                Obtenez des devis de professionnels qualifiés directement via la plateforme.
              </p>
            </div>
            <button
